...
# No changes needed for normalize_answer
...
def _words_to_int(phrase: str) -> int | None:
    ...
    val, used = 0, False
    for w in tokens[::-1]:  # 从尾到头取最近的一段数字词
        if w in _NUM_0_19:
...
def extract_final_answer(text: str) -> str | None:
    ...
    # 3) 全文显式短语 / 等号
    m = re.findall(r'(?:final\s+answer|answer|result|value)\s*(?:is|:)\s*' + num_pat,
                   text, flags=re.IGNORECASE)
    if m: return m[-1][-1]
    m = re.findall(r'=\s*' + num_pat, text)
    if m: return m[-1]

    # 4) 全文最后一个数字
    m = re.findall(num_pat, text)
    if m: return m[-1]

    # 5) 英文数字词
    w = re.findall(rf'\b{_WORD_RE}(?:[-\s]{_WORD_RE})?\b', text, flags=re.IGNORECASE)
    if w:
        val = _words_to_int(w[-1])
        if val is not None:
            return str(val)
...
#     """
#     [FINAL VERSION V7] A robust function for comparing math answers.
    
#     This version follows a strict logical order to maximize accuracy:
#     1.  Extract and clean the predicted and ground truth answers.
#     2.  Check for an exact normalized string match (covers integers, simple strings).
#     3.  If no exact match, attempt a robust numerical comparison for floats.
#     4.  If not numerical, attempt a structural comparison for matrices/tuples.
#     """
...
#     # Step 3: Normalize both cleaned strings for comparison.
#     pred_norm = normalize_answer(pred_clean)
#     gt_norm = normalize_answer(gt_clean)
    
#     # Step 4: Check for an EXACT string match first. This is the most reliable check.
#     # This will correctly handle integers, simple fractions, and non-numeric answers.
#     if pred_norm == gt_norm:
#         return True

#     # Step 5: If string match fails, attempt a more lenient NUMERICAL comparison.
#     # This handles cases like 23.4 vs 23, or 336.0 vs 336.
#     try:
#         # Avoid this path if the answer contains non-numeric characters (except for .,-/)
#         if re.search(r'[a-zA-Z]', pred_norm) or re.search(r'[a-zA-Z]', gt_norm):
#             pass
#         else:
#             # Convert to float for comparison
#             pred_val = float(pred_norm.replace(',', '').split('/')[0]) / float(pred_norm.replace(',', '').split('/')[1]) if '/' in pred_norm else float(pred_norm.replace(',', ''))
#             gt_val = float(gt_norm.replace(',', '').split('/')[0]) / float(gt_norm.replace(',', '').split('/')[1]) if '/' in gt_norm else float(gt_norm.replace(',', ''))
            
#             # Check for close floating point values OR if the prediction rounds to the ground truth integer
#             if math.isclose(pred_val, gt_val, rel_tol=1e-4, abs_tol=1e-4):
#                 return True
#             if abs(gt_val - round(gt_val)) < 1e-9 and int(round(pred_val)) == int(gt_val):
#                 return True
#     except (ValueError, TypeError, ZeroDivisionError, OverflowError):
#         # If conversion fails, it's not a standard number, so we move on.
#         pass

#     # Step 6: If all else fails, attempt a STRUCTURAL comparison for matrices/tuples.
#     # This is a fallback for complex structures that might fail simple normalization.
...
#         # Extract just the numbers from the structures and compare them as lists.
...
def is_correct(generated_solution: str, ground_truth_solution: str) -> bool:
    pred_raw = extract_final_answer(generated_solution)
    gt_raw = extract_final_answer(str(ground_truth_solution)) or str(ground_truth_solution)
    if pred_raw is None or gt_raw is None:
        return False

    # 可选：保留你的 _clean_and_validate_answer
    # pred_clean = _clean_and_validate_answer(pred_raw)
    # gt_clean = _clean_and_validate_answer(gt_raw)
    pred_clean, gt_clean = pred_raw, gt_raw

    pred = normalize_answer(pred_clean)
    gt = normalize_answer(gt_clean)
    ...
    try:
        ...
        if math.isclose(pv, gv, rel_tol=1e-4, abs_tol=1e-4):
            return True
        # 真值是整数时允许四舍五入一致
        if abs(gv - round(gv)) < 1e-9 and int(round(pv)) == int(round(gv)):
            return True
    except Exception:
        pass
    ...
    # 3) 结构比较（矩阵/向量）
    structural_pattern = r'[\(\[].*?[\)\]]|\\begin{(?:p|b)matrix}.*?\\end{(?:p|b)matrix}'
    if re.search(structural_pattern, pred_clean, re.DOTALL) and re.search(structural_pattern, gt_clean, re.DOTALL):
        pred_nums = re.findall(r'-?\d+\.?\d*', pred_clean)
        gt_nums = re.findall(r'-?\d+\.?\d*', gt_clean)
        if pred_nums and gt_nums and pred_nums == gt_nums:
            return True

    # （可选，默认关闭）若预测里出现多个数字而真值为单数，可放开“包含匹配”
    # nums_in_pred = re.findall(r'-?\d+(?:\.\d+)?(?:/\d+)?', pred_clean)
    # if len(nums_in_pred) > 1 and normalize_answer(gt_clean) in [normalize_answer(x) for x in nums_in_pred]:
    #     return True
