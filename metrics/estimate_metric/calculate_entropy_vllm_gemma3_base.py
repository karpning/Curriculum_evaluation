...
OUTPUT_FILE = 'outputs/Gemma_4B_base_entropy_20K_metrics_512.jsonl' 
FAILURE_LOG_FILE = 'outputs/Gemma4B_base_entropy_20K_failed_512.log' 
K = 20
...
def format_prompt(item: Dict[str, Any]):
    """
    Formats a simple completion prompt with CoT for a base model.
    """
    problem_query = item['query']
...
def calculate_shannon_entropy(logprobs: List[float]) -> float:
    ...
    # Use log-sum-exp trick to compute the normalization constant in log-space
    max_lp = max(logprobs)
    # This check avoids math.log(0) if all logprobs are -inf
    try:
    ...
    entropy = 0.0
    for lp in logprobs:
        # We only consider finite log probabilities
        if lp > -float('inf'):
            # p = exp(lp - log_sum_exp)
            # log(p) = lp - log_sum_exp
            # The term in the entropy sum is -p * log2(p)
            # -p * (lp - log_sum_exp) / math.log(2)
            p = math.exp(lp - log_sum_exp)
            if p > 0:
                entropy -= p * (lp - log_sum_exp) / math.log(2) # Convert from nats to bits
    return entropy
...
async def get_completions_with_logprobs(item: Dict[str, Any], client: AsyncOpenAI, n: int):
    ...
    try:
        ...
        # Wrap each choice in a dummy object that mimics the expected response structure
    ...
    async def process_item(item: Dict[str, Any]):
        ...
        async with semaphore:
            try:
                ...
                # --- Lists to hold all metrics ---
                nll_scores, nll_per_token_scores, logit_gap_means = [], [], []
                total_entropy_scores, mean_entropy_per_token_scores = [], []
                # This list aggregates all per-token entropies across all K samples
                # to calculate a global average token uncertainty for the prompt.
                ...
                for response in responses:
                    ...
                    # Filter out potential None values from token_logprobs for robustness.
                    valid_token_logprobs = [lp for lp in logprobs_obj.token_logprobs if lp is not None]
                    # [MODIFIED] Added warning for better debugging
                    ...
                    # [MODIFIED] Safer iteration to prevent index mismatches between
                    # token_logprobs and top_logprobs.
                    if logprobs_obj.top_logprobs:
                        valid_indices = [i for i, lp in enumerate(logprobs_obj.token_logprobs) if lp is not None]
                        for i in valid_indices:
                            # Ensure the index exists in top_logprobs and the entry is not empty
                            if i < len(logprobs_obj.top_logprobs) and logprobs_obj.top_logprobs[i]:
                                
                                # ==================== [START] CODE FIX ====================
                                # 修复：vLLM可能返回一个(token, logprob)元组的列表，而不是一个字典。
                                # 此代码块检查数据类型并将其安全地转换为字典。
                ...
                # --- Assign ALL requested metrics to the item dictionary ---
                ...
                # Global mean/variance of all per-token entropies across all K sequences.
                ...
                # Total entropy of a full sequence, averaged over all K sequences.
                ...
                # Average per-token entropy of a full sequence, averaged over all K sequences.
